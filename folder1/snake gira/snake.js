...
/* SNAKE OBJECT. Declare Snake Class*/
function Snake(){
	//grid position
	this.vx = 1;
	this.vy = 0;
	...
	this.update = function(){
		...
		// GIRAR EN LA PARED
		// Hacia derecha
		if(this.boxes[0].x == (GRIDX-1) && this.vx == 1){
			// Entonces girar arriba.
			this.vx = 0;
			this.vy = -1;
		} else if(this.boxes[0].y == 0 && this.vy == -1){
			// Entonces girar izquierda.
			this.vx = -1;
			this.vy = 0;
		} else if(this.boxes[0].x == 0 && this.vx == -1){
			// Entonces girar abajo.
			this.vx = 0;
			this.vy = 1;
		} else if(this.boxes[0].y == (GRIDY-1) && this.vy == 1){
			// Entonces girar derecha.
			this.vx = 1;
			this.vy = 0;
		}
